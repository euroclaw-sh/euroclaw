...
/** Read the env global the way `env()` does — used only to prove the default-source path. */
const envGlobal = (): Record<string, string | undefined> | undefined =>
	(globalThis as { process?: { env?: Record<string, string | undefined> } })
		.process?.env;

describe("env — the environment provider", () => {
	it("reads a value out of its source as token material", async () => {
		const provider = env({ source: { GITHUB_TOKEN: "ghp_abc" } });
	...
	it("returns null for a missing key (env var unset)", async () => {
		expect(await env({ source: {} }).get("MISSING", {})).toBeNull();
	});

	it("returns null for an explicitly-undefined key", async () => {
		expect(await env({ source: { EMPTY: undefined } }).get("EMPTY", {})).toBeNull();
	});
	...
	it("defaults source to the env global (globalThis.process.env)", async () => {
		...
		store[key] = "probe";
		try {
			expect(await env().get(key, {})).toEqual({ kind: "token", value: "probe" });
		} finally {
			delete store[key];
		}
	});
...
describe("buildSecrets — the one-door resolver", () => {
	...
	it("resolves down the chain — the first non-null provider wins", async () => {
		const secrets = buildSecrets([
			env({ name: "a", source: { SHARED: "from-a" } }),
			env({ name: "b", source: { SHARED: "from-b", ONLY_B: "b-only" } }),
		]);
		expect(await secrets.get("SHARED")).toEqual({ kind: "token", value: "from-a" });
		expect(await secrets.get("ONLY_B")).toEqual({ kind: "token", value: "b-only" });
	...
	it("applies a provider's aliases (canonical → backend key); pass-through when unaliased", async () => {
		const secrets = buildSecrets([
			env({
				source: { PROD_TELEGRAM: "prod-token", GITHUB_TOKEN: "gh" },
		...
		expect(await secrets.get("GITHUB_TOKEN")).toEqual({ kind: "token", value: "gh" });
	...
	it("remaps per provider — each provider's own aliases apply to its own get", async () => {
		const secrets = buildSecrets([
			env({ name: "a", source: {}, aliases: { API_KEY: "A_KEY" } }), // A_KEY unset → miss
			env({ name: "b", source: { B_KEY: "from-b" }, aliases: { API_KEY: "B_KEY" } }),
		]);
		// A remaps API_KEY→A_KEY (miss), then B remaps API_KEY→B_KEY (hit)
		expect(await secrets.get("API_KEY")).toEqual({ kind: "token", value: "from-b" });
	});
	...
	it("forwards the remapped key and ctx to the provider", async () => {
		...
		await buildSecrets([spy]).get("CANON", { organizationId: "org_1", actor: "user_1" });
	...
	it("has — true iff some provider resolves the name", async () => {
		const secrets = buildSecrets([env({ source: { PRESENT: "v" } })]);
