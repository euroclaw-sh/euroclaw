...
describe("assertEgressAllowed", () => {
	...
	it("fails loud when a named host needs resolution but no lookup is injected", async () => {
		await expect(assertEgressAllowed("https://example.com/x")).rejects.toThrow(
			/no DNS lookup injected/,
		);
	});
});
