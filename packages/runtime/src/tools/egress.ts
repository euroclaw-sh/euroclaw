...
// cannot see network reality, so before a registered tool's request opens a socket this floor:
...
//
// Residual gap (documented, no new dependency): Node's global `fetch` exposes no per-request hook
// to force the connection onto a pre-resolved IP without an undici `Agent` (a dependency §0.3
// forbids). So while the floor pins and returns the vetted address, the DEFAULT global fetch still
// re-resolves at socket time. A host that needs a hard pin injects a `fetch` bound to a dispatcher
// built from `pinnedAddress`. IP-literal targets (no DNS) have no residual gap at all.
...
/** Resolve a hostname to its addresses. Injectable so tests never touch real DNS and hosts can
 *  supply a caching / pinning resolver. */
...
export type EgressOptions = {
	...
	/** Override DNS resolution (tests inject a fake; default resolves via node:dns). */
	lookup?: EgressLookup;
};
...
export async function assertEgressAllowed(
	...
	// An IP-literal host is validated directly — no DNS, so no rebinding window at all.
	const resolved: readonly ResolvedAddress[] = literal
